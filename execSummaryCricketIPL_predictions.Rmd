...
One of the ways to measure success of predicting a continuous variable, End of Innings Score, is to measure the Root Mean Square Errror of the predictions made at various stages of each of the two innings during a match. While for the categorical variable, Win or Lode prediction, I classified it using a threshold score from the prediction model, and plotted the accuracy of prediction at various stages of the match, as shown below. 
...
3. Better feature enginnering to account for venue city: consider adding a column that will reflect if Team A was playing on its home ground, and compute stats for winning record on home and away grounds and store them as independent variables for the prediction models. 
