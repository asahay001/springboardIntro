...
1. New columns created for data exploration and liner regression models: 
    ...
    * Matches won by each team i nthe data set
...
1. Running a correlation fucntion for the Over stats (independent variables) against the depenent variable, *Runs Scored By A Team in an innings* shows that correlation with EOI Runs (End of the Innings score) is higher as the number of overs played increases. Also, *Runs scored* has a *positive correlation with EOI Runs*, while *Wickets lost* has a *negative impact* on the End of Innings score:
...
```{r MatchesWon, echo=FALSE, warning=FALSE}
...
```{r BeforeMatchStartToss, echo=FALSE, warning=FALSE}
ggplot (data = matSumm) +
...
```{r BeforeMatchStartPastResult, echo=FALSE, warning=FALSE}
...
```{r DuringMatch_Runs, echo=FALSE, warning=FALSE}
...
```{r DuringMatch_Wkts, echo=FALSE, warning=FALSE}
